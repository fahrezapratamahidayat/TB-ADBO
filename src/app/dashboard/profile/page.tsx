...
export default function ProfilePage() {
  ...
  const handleUpdateUser = async (e: any) => {
    ...
    try {
      if (selectedImage) {
        ...
        const storageRef = ref(storage, `image/profile/${name}`);
          ...
          async () => {
            try {
              ...
              const res = await fetch(
                `/api/user/updateuser?id=${session?.user?.id}`,
                {
                  method: "PUT",
                  headers: {
              ...
              setProfile(updatedProfileData.data);
              e.target.reset();
            } catch (error) {
              console.error(error);
  ...
  return (
    <>
      <div
        className={`bg-primary min-h-screen relative w-full ${
          edit ? "hidden" : "block"
        }`}
      >
        ...
        <div className="w-full flex items-center px-5 ">
          ...
          <h1 className="text-[25px] font-bold ml-[25px]">Profile</h1>
        </div>
        <div className="flex items-center w-full flex-col">
          ...
          <div className="flex flex-col w-full items-center">
            <h1 className="mt-5 font-bold text-base">Change Profile</h1>
          </div>
        </div>
        <div className={`px-5 mt-3 }`}>
          <h1 className="text-black text-[18px] font-bold">Personal Data</h1>
          <div className="flex flex-col">
            <h2 className="ml-5 text-base font-bold">Nama</h2>
            <div className="mt-2 w-[21.4375rem] bg-white h-[4rem] rounded-[2rem] flex items-center ">
              <h2 className="ml-8">{profile && profile.username}</h2>
            </div>
          </div>
          <div className="flex flex-col">
            <h2 className="ml-5 text-base font-bold">Jenis Kelamin</h2>
            <div className="mt-2 w-[21.4375rem] bg-white h-[4rem] rounded-[2rem] flex items-center ">
              <h2 className="ml-8">{profile && profile.gender}</h2>
            </div>
          </div>
          <div className="flex flex-col">
            <h2 className="ml-5 text-base font-bold ">Nomor</h2>
            <div className="mt-2 w-[21.4375rem] bg-white h-[4rem] rounded-[2rem] flex items-center ">
              <h2 className="ml-8">{profile && profile.phone}</h2>
            </div>
          </div>
          <div className="flex flex-col">
            <h2 className="ml-5 text-base font-bold">Email</h2>
            <div className="mt-2 w-[21.4375rem] bg-white h-[4rem] rounded-[2rem] flex items-center ">
              <h2 className="ml-8">{profile && profile.email}</h2>
            </div>
          </div>
          <div className="flex items-center justify-around w-full pb-5">
            <button
              onClick={() => setEdit(!edit)}
              className="w-[5.12rem] text-white bg-black h-[1.75rem] rounded-[2rem] flex items-center justify-center mt-5"
            >
              Edit
            </button>
            <button className="w-[5.12rem] text-white bg-black h-[1.75rem] rounded-[2rem] flex items-center justify-center mt-5">
              Simpan
            </button>
          </div>
        </div>
      ...
      >
        ...
        <form action="" onSubmit={handleUpdateUser}>
          ...
          <div className={`px-5 mt-3 ${edit ? "block" : "hidden"}`}>
            <h1 className="text-black text-[18px] font-bold">Personal Data</h1>
            <div className="flex flex-col">
              <h2 className="ml-5 text-base font-bold">Nama</h2>
              <div className="mt-2 w-[21.4375rem] bg-white h-[4rem] rounded-[2rem] flex items-center ">
                <input
                  type="text"
                  className=" w-full h-full rounded-[2rem] placeholder:pl-8 focus:pl-8 focus:placeholder:pl-0 pl-5"
                  id="name"
                  name="name"
            ...
            <div className="flex flex-col">
              <h2 className="ml-5 text-base font-bold">Jenis Kelamanin</h2>
              <div className="mt-2 w-[21.4375rem] bg-white h-[4rem] rounded-[2rem] flex items-center ">
                <input
                  type="text"
                  className=" w-full h-full rounded-[2rem] placeholder:pl-8 focus:pl-8 focus:placeholder:pl-0 pl-5"
